...
// popula a tabela com os itens iniciais da roleta
...
function posicionaFatias() {
  ...
  const configTexto = {
    3: `rotate(49deg) translateY(10%) skewX(7deg)`,
    4: `rotate(45deg) translateY(-20%)`,
    5: `rotate(45deg) translateY(-40%)`,
    6: `rotate(45deg) translateY(-20%) skewX(4deg)`,
    7: `rotate(49deg) translateY(30%) skewX(9deg)`,
    8: `rotate(50deg) translateY(-30%) skewX(17deg)`,
  };

  itens.forEach((item, i) => {
    const texto = item.querySelector(".texto-item");

    // reseta estilos para não acumular estilos de configurações anteriores
    item.style = "";
    texto.style = "";
    ...
    // ajustes manuais de posicionamento do texto por quantidade de fatias
    if (total === 2) {
      item.style.top = '0';
      item.style.left = '0';
    ...
    if (total !== 2 && configTexto[total]) {
      texto.style.transform = configTexto[total];
    }
  });
}
posicionaFatias();
...
function girarRoleta() {
  ...
  const deslocamentoDeBorda = 23;
  const rotacoes = 5;
  const indice = Math.floor(Math.random() * total);
  const anguloFatia = indice * angulo;
  const ajusteAngular = total === 2 ? anguloFatia : ((180 - anguloFatia) + deslocamentoDeBorda);
  const rotacao = (360 * rotacoes) + ajusteAngular;

  const resultTexto = document.getElementById("result");
  resultTexto.style.visibility = "hidden";

  // força o navegador a resetar para a posição inicial antes de iniciar nova animação
...
function adicionarComida(event) {
  ...
  // cria novo item <li> e adiciona na roleta
...
function visibilidadeFormulario() {
  ...
  if (flexDirection === "column") {
    // mobile: scrolla até o container do formulário
    window.scrollTo(0, window.innerHeight);
  } else {
    // desktop: alterna visibilidade do container lateral
